...
- After flattening the row renderer, `layout` had become a redundant prop on each row view because row types already encode split vs stack. Removing that prop/comparison produced another small but measurable gain.
- `renderInlineSpans` was still doing an extra reduce pass after trimming spans. Computing `usedWidth` during trimming removed that second walk from the hottest text-render path and produced a clear win.
